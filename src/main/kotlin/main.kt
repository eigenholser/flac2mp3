...
fun main(args: Array<String>) {
    ...
    var currentFlacAlbum = ""
    var prevFlacAlbum = ""
    var currentMP3AlbumPath = ""
    var prevMP3AlbumPath = ""
    ...
    File(Config.flacRoot)
        ...
        .forEach {
            ...
            if (currentFlacAlbum.compareTo(it.flacAlbumPathAbsolute.toString()) != 0) {
                ...
                logger.info("Is PNG album art present: ${albumArtPNGExists(it)}")
                if (albumArtPNGExists(it)) {
...
//            logger.info("Previous album: $prevFlacAlbum")
//            logger.info("Current album: $currentFlacAlbum")
//            logger.info("Previous MP3 album: $prevMP3AlbumPath")
            if (!mp3FileExists(it) || !isTrackCurrent(it)) {
                ...
                if (isAlbumArtUpdated(it)) {
                    Tag.updateAlbumArtField(
                        it.mp3FileAbsolute.toString(),
                        it.mp3AlbumPathAbsolute.toString()
                    )
//                    logger.info("Passing ${it.mp3FileAbsolute} and ${it.mp3AlbumPathAbsolute} to updateAlbumArtField")
                }
            }
...
//            }
    ...
    // Delete the last album art
        if (File(currentMP3AlbumPath).toPath().resolve(Config.coverArtFile).exists()) {
            deleteMp3CoverArt(File(currentMP3AlbumPath).toPath())
        }
}
